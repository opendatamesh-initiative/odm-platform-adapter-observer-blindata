...
| **Max batch size** | **500**, as an **API request argument** (not application/yml configuration). API rejects more than 500 paths. Observer chunks within the current unit of work if an entity has more than 500 linked fields. |
...
| **Response shape** | **Confirmed: wrapper resource class**, not a bare JSON array. Must stay extensible (counts, errors, extra metadata later). Per-path outcomes live inside that class. |
...
- **CSV physical-field semantic-link import**: In-process bulk on the API: each CSV row with a path string already calls the same resolve-field service in a loop, catching per-row not-found/bad-request and accumulating errors. Proves per-path failure isolation is an established resolve convention; it is not an HTTP bulk API and it persists catalog links rather than returning resolved objects to a remote client.
...
- **Bulk resolve-field operation**: A single Blindata API call that accepts many semantic paths (each with a default namespace), resolves each with the same semantics as today’s single-path utility, and returns a **wrapper result resource** with per-path outcomes. Sibling of the existing GET, not a replacement.
...
- **Per-path resolve outcome**: For each requested path, either a resolved logical-field+semantic-link (same meaning as today) or an unresolved/failed outcome that does not fail the rest of the batch. Carried inside the wrapper result class so the shape can grow (e.g. error message, counts).
...
- **Max batch size is 500, as an API argument**: Not configurable in application settings. Oversized requests are rejected by the API; the observer never sends more than 500 paths.
...
- **Partial success inside a batch**: One bad path must not discard successful resolutions in the same batch. HTTP success for a well-formed request; failures are per path inside the wrapper result.
...
On the API: keep GET `*/resolvefield` as-is. Introduce a collection-oriented resolve operation on the same semantic-linking utils surface that reuses the current path-parsing/resolution logic per path, takes/enforces a max of **500** via a request argument, and returns a **wrapper result class** with per-path success or failure. Internally this is still “resolve this path,” not a new glossary algorithm. The endpoint is synchronous; generic async request/poll remains available to any client, including the observer when `enableAsync` is true.
...
- **Partial results in a wrapper class**: **Confirmed.** HTTP success for a well-formed batch; per-path unresolved paths inside an extensible result resource (same family as `QualitySuiteImportResultRes` / `SemanticLinksRes` / `ResponseMessage`). Observer maps failures to `[#90]`-class warnings without aborting sibling fields.
...
- **`enableAsync` for resolve**: Honor the existing flag the same way as quality/port-assets. With 500 paths, `enableAsync=false` can still time out; that is the same trade-off as other bulk writes.
...
- **Mixed namespaces in one batch**: Unusual for one observer entity (`s-base` is shared) but valid if the bulk API is general-purpose. Must allow per-path default namespace.
...
- **All paths in a batch fail to resolve**: Batch HTTP should still succeed; observer logs a warning per path and leaves fields unmodified (today’s `[#90]` intent).
...
- **Observer `withErrorHandling` abort**: A transport/5xx failure on a bulk call still fails the whole entity’s remaining enrichment. Mitigation: treat bulk like quality upload (propagate 5xx); keep per-path glossary failures inside the wrapper payload so they no longer abort siblings.
- **Poll exhaustion**: Async poll has a finite attempt budget with backoff. A 500-path batch that runs longer than that budget fails as “polling exceeded maximum attempts.” Mitigation: request-scoped lookup reuse if profiling shows cost; do not raise the cap.
...
- **Behavioral change for first-failure**: Products that previously stopped linking after the first bad path may start linking later fields on the same entity. That is intended with per-path outcomes; tests and docs should call it out.
...
| 7 | Unresolvable paths remain non-blocking for other fields; result is an extensible wrapper class | Yes | Per-path outcomes inside a resource class, not a raw list. |
